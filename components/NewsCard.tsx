...
const NewsCard: React.FC<NewsCardProps> = ({ article, onClick }) => {
  ...
  return (
    <article className="group bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm hover:shadow-xl hover:border-gray-300 transition-all duration-300 hover:-translate-y-1">
      <div
        role="button"
        tabIndex={0}
        onClick={handleOpen}
      ...
      >
        ...
        <div className="p-5">
          <h3 className="text-lg font-bold text-gray-900 mb-2.5 line-clamp-2 group-hover:text-red-600 transition-colors duration-200 leading-snug">
            {title}
          </h3>
          {article.description && (
            <p className="text-sm text-gray-600 mb-4 line-clamp-3 leading-relaxed">{article.description}</p>
          )}
          <div className="flex items-center justify-between pt-3 border-t border-gray-100">
            ...
            <a
              ...
              className="ml-3 px-4 py-1.5 rounded-lg text-xs font-bold bg-red-600 text-white hover:bg-red-700 hover:shadow-lg hover:shadow-red-600/30 transition-all duration-200 active:scale-95"
              onClick={(e) => {
                e.stopPropagation();
              }}
